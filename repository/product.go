...
func (pr *ProductRepository) Order(p *model.Product) (int, error) {
	res, err := pr.pgdb.Model(p).
		Set("quantity = product.Quantity - ?", p.Quantity).
		Where("product_id = ? AND quantity >= ?", p.Id, p.Quantity).
		Update()
...
func (pr *ProductRepository) Store(p *model.Product) (int, error) {
	res, err := pr.pgdb.Model(p).
